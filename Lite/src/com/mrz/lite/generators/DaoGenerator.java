...
public class DaoGenerator implements Generator {
	...
	public void generate(JavaFileObject jfo, EntityModel entityModel) {
		try {
			...
			bw.append("        db = liteCore.getReadableDatabase();");
			bw.newLine();
			bw.append("        Cursor cursor = db.rawQuery(\"SELECT * FROM '\"+ entityClazz.getSimpleName() +\"' WHERE '\"+ fieldName +\"' = '\"+ fieldValue +\"'\", null);");
			bw.newLine();
			bw.append("        db.close();");
			bw.newLine();
			bw.append("        return (List<T>) instanceConstructor(cursor);");
			bw.newLine();
			bw.append("    }");
			bw.newLine();
			bw.newLine();
